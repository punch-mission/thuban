...
from thuban.catalog import find_star_coordinates_in_image
...
def _residual(params: Parameters,
              observed_coords: np.ndarray, catalog: pd.DataFrame, guess_wcs: WCS,
              x_lim: (int, int) =(0, 2048), y_lim: (int, int) =(0, 2048), n: int =30):
    ...
    refined_coords = find_star_coordinates_in_image(catalog, refined_wcs, x_lim=x_lim, y_lim=y_lim)
    observed_coords, refined_coords = remove_pairless_points(observed_coords, refined_coords, 25)
    ii = np.min([np.array(list(range(n))), np.ones(n) * observed_coords.shape[0] - 1], axis=0).astype(int)
    return observed_coords[ii] - refined_coords[ii]


def refine_pointing(image, guess_wcs, file_index, observed_coords, catalog,
                    background_width=16, background_height=16,
                    detection_threshold=5,
                    x_lim=(0, 2048), y_lim=(0, 2048), n=30):
    ...
    object_positions = np.stack([objects["x"], objects["y"]], axis=-1)
    ...
    # optimize
    try:
        out = minimize(_residual, params, args=(object_positions, catalog, guess_wcs, x_lim, y_lim, n))
    except IndexError:
    ...
    return result_wcs, file_index, observed_coords
...
def get_star_lists(catalog, refined_wcs, observed_coords, max_distance=20) -> (np.ndarray, np.ndarray, np.ndarray):
    ...
    """
    refined_coords = find_star_coordinates_in_image(catalog, refined_wcs, mode='all')
    no_distortion = find_star_coordinates_in_image(catalog, refined_wcs, mode='wcs')
